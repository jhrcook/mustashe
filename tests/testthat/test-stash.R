test_that("stashing works", {
    ...
    expect_true(file.exists(file.path(target_dir, "a.rds")))
    ...
    expect_null(stash("a", { a <- 1 }))
    expect_equal(a, 1)
    expect_equal(file_time(file.path(target_dir, "a.rds")), old_rds_time)
    expect_equal(file_time(file.path(target_dir, "a.hash")), old_hash_time)

    expect_null(stash("a", { a <- 2 }))
    expect_equal(a, 2)
    expect_false(file_time(file.path(target_dir, "a.rds")) == old_rds_time)
    ...
    expect_true(file.exists(file.path(target_dir, "a.rds")))
    expect_true(file.exists(file.path(target_dir, "a.hash")))
    expect_false(file.exists(file.path(target_dir, "b.rds")))
