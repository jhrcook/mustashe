...
The goal of 'stashR' is to save time on long-running computations by storing and reloading the resulting object after the first run.
The next time the computation is run, instead of evaluating the code, the stashed object is loaded.
'stashr' is great for storing intermediate objects in an analysis.
...
And the development version from [GitHub](https://github.com/jhrcook/stashR) with:
...
The 'stashR' package is loaded like any other, using the `library()` function.
...
Below is a simple example of how to use the `stash()` function from 'stashR'.
