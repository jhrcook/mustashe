test_that("the stash gets cleared", {
    ...
    file.create(c(rds_file, hash_file, other_file))

    expect_true(all(file.exists(c(rds_file, hash_file, other_file))))
    expect_message(clear_stash(), "Clearing stash")
    expect_false(any(file.exists(c(rds_file, hash_file))))
